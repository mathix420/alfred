...
/*
 * ALFRED: starter LVGL config for the companion firmware.
 *
 * Derived verbatim from LVGL 9.5.0's lv_conf_template.h with the enable guard
 * flipped to 1 and LV_COLOR_DEPTH left at 16 (RGB565 — matches the AMOLED panel).
 * Everything else is stock LVGL defaults so the firmware links today; the UI
 * workstream owns the real tuning (fonts, widgets, draw buffers, FreeRTOS OS
 * integration via LV_USE_OS, memory pool size). It is found via
 * `-DLV_CONF_INCLUDE_SIMPLE` (platformio.ini) + the `-I src` include path.
 *
...
/* clang-format off */
#if 1 /* Enabled for the Alfred companion build (was 0 in the template). */
...
#define LV_FONT_MONTSERRAT_16 1  /* ALFRED: ALF_FONT_TERM (theme.h) */
#define LV_FONT_MONTSERRAT_18 0
#define LV_FONT_MONTSERRAT_20 1  /* ALFRED: ALF_FONT_LABEL (theme.h) */
...
#define LV_FONT_MONTSERRAT_48 1  /* ALFRED: ALF_FONT_CLOCK (theme.h) */
